...
# ================== CONFIG ==================
TOKEN = "8089913517:AAFpc7t_Nf_F9pmY8X3DHmSQtyz0kit9_gE"  # bu yerga haqiqiy tokeningni yoz
ADMIN_ID = 1081829777
DB_NAME = "shop.db"
...
async def show_catalog(message: types.Message):
    ...
    if not products:
        ...
        return
    for p in products:
        btn = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(f"➕ {p[1]}")]], resize_keyboard=True)
...
async def search_process(message: types.Message, state: FSMContext):
    ...
    if results:
        for r in results:
            btn = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(f"➕ {r[1]}")]], resize_keyboard=True)
...
async def admin_delete_product(message: types.Message):
    ...
    if not products:
        await message.answer("Hozircha mahsulotlar yo‘q.")
        return
    text = "O‘chirmoqchi bo‘lgan mahsulotni tanlang:\n"
    for p in products:
        text += f"{p[1]}\n"
    await message.answer(text)

@dp.message(F.text.startswith("O'chirish: "))
async def admin_delete_product_process(message: types.Message):
    product_name = message.text.replace("O'chirish: ", "")
    delete_product(product_name)
    await message.answer(f"{product_name} o‘chirildi!", reply_markup=admin_menu)
...
async def admin_orders(message: types.Message):
    ...
    text = ""
    for o in orders:
        text += f"ID:{o[0]} {o[2]} ({o[3]}, {o[4]}) - {o[5]} - {o[6]} so'm - {o[7]}\n"
    await message.answer(text)
